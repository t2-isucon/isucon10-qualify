...
do
  ...
  ssh isucon@${srv} git -C ${repo_path} switch master || ssh isucon@${srv} git clone https://github.com/t2-isucon/isucon10-qualify.git
  ssh isucon@${srv} git -C ${repo_path} pull origin master
  ssh isucon@${srv} git -C ${repo_path} fetch --all
  ssh isucon@${srv} git -C ${repo_path} branch -D ${branch} || true
